...
const URL_RE = /(https?:\/\/(?:localhost|127\.0\.0\.1|\[::1\]):(\d+)\/?[^\s\x1b]*)/gi;

// Vite (and other dev-server CLIs) colorize the "Local: http://..." line with
// ANSI SGR codes, sometimes splitting the URL itself — e.g. it emits
// `http://localhost:\x1b[1m5181\x1b[22m/` so the port digits are not adjacent
// to the colon in the raw buffer. Strip escapes before matching.
...
/**
 * Find the dev server's ready URL in the accumulated output buffer.
 * Vite retries on the next port when its preferred one is busy, printing
 * "Port NNNN is in use, trying another one..." lines before the real
 * "Local: http://..." line — those retry lines aren't URL-shaped so they
 * don't false-match, but to be robust against any tool that does emit an
 * incidental URL-shaped string earlier in the log, take the LAST match in
 * the buffer rather than the first: the most recent ready URL wins.
 */
function findServerUrl(buffer) {
...
async function startServer(opts) {
  ...
  const url = await new Promise((resolve, reject) => {
    ...
    const scan = (d) => {
      buffer += d.toString();
      const found = findServerUrl(buffer);
...
function stopServer(proc) {
  ...
  try {
    if (process.platform === 'win32') {
      // `proc` is `npm.cmd` under a shell; `proc.kill()` reaps only that wrapper
      // and orphans the vite NODE child. `taskkill /T` kills the whole tree,
      // `/F` forces it — otherwise the harness hangs on the live child.
      spawnSync('taskkill', ['/F', '/T', '/PID', String(proc.pid)], { stdio: 'ignore' });
    } else {
      process.kill(-proc.pid, 'SIGTERM'); // detached: kill the process group
    }
  } catch { /* already gone */ }
}
